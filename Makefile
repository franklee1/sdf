...
pkgdatadir = $(datadir)/unodecore
pkgincludedir = $(includedir)/unodecore
pkglibdir = $(libdir)/unodecore
pkglibexecdir = $(libexecdir)/unodecore
...
CONFIG_HEADER = $(top_builddir)/src/config/unode-config.h
CONFIG_CLEAN_FILES = libunodeconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libunodeconsensus.pc.in \
	...
	$(top_srcdir)/src/config/unode-config.h.in \
...
ACLOCAL = ${SHELL} /root/unode/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /root/unode/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /root/unode/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /root/unode/build-aux/missing automake-1.15
AWK = gawk
BDB_CPPFLAGS = 
...
LUPDATE = 
MAINT = 
MAKEINFO = ${SHELL} /root/unode/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = unodecore
PACKAGE_BUGREPORT = https://github.com/unodepay/unode/issues
PACKAGE_NAME = unode Core
PACKAGE_STRING = unode Core 1.2.1
PACKAGE_TARNAME = unodecore
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = 
abs_builddir = /root/unode
abs_srcdir = /root/unode
abs_top_builddir = /root/unode
abs_top_srcdir = /root/unode
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /root/unode/build-aux/install-sh
...
pkgconfig_DATA = libunodeconsensus.pc
BITCOIND_BIN = $(top_builddir)/src/unoded$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/unode-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/unode-cli$(EXEEXT)
BITCOIN_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
OSX_APP = unode-Qt.app
OSX_DMG = unode-Core.dmg
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_unode_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_unode_coverage.info test_unode.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/unode-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/unode-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/unode-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/unode-config.h
$(top_srcdir)/src/config/unode-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/unode-config.h src/config/stamp-h1
libunodeconsensus.pc: $(top_builddir)/config.status $(srcdir)/libunodeconsensus.pc.in
...
$(OSX_APP)/Contents/MacOS/unode-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/unode-Qt

$(OSX_DMG): $(APP_DIST_EXTRAS)
	$(GENISOIMAGE) -no-cache-inodes -D -l -probe -V "unode-Core" -no-pad -r -dir-mode 0755 -apple -o $@ dist
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/unode-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_unode.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_unode -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_unode_filtered.info: test_unode.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_unode_filtered.info
#	$(MKDIR_P) qa/tmp
...
#rpc_test.info: test_unode_filtered.info
...
#test_unode_coverage.info: baseline_filtered_combined.info test_unode_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_unode_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_unode_filtered.info block_test_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_unode_filtered.info -a block_test_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_unode.coverage/.dirstamp:  test_unode_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_unode.coverage/.dirstamp total.coverage/.dirstamp

# unode: disable the test until upstream switches from convoluted comparison tool
...
clean-local:
	rm -rf coverage_percent.txt test_unode.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
